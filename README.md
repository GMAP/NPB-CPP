# The NPB-CPP Benchmark Suite

The NPB's Fortran codes were carefully ported to **C++** from the original **NPB3.3.1** with optimizations that were introduced in the new **NPB3.4.1** version ([NPB official webpage](https://www.nas.nasa.gov/publications/npb.html)). Our paper under review for the TPDS journal contains abundant information on how the porting was conducted and discusses the outcome performance we obtained with **NPB-CPP** on different machines (Intel Xeon, AMD Epyc and IBM Power8) and compilers (GCC and ICC). Results showed that we achieved similar performance with **NPB-CPP** compared to the original **NPB**. **You can use our paper, along with the official reports, as a guide to access performance using the NPB suite**.
