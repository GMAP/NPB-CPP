...
int main(int argc, char **argv){
	...
	if(size[j]=='.'){j--;}
	size[j+1] = '\0';
	printf("\n\n NAS Parallel Benchmarks 4.1 Parallel C++ version with SPar - EP Benchmark\n\n");
	...
	[[spar::ToStream, spar::Input(an, k_offset, np,timers_enabled, sx, sy)]]
	for(k=1; k<=np; k++){
		[[spar::Stage, spar::Pure, spar::Input(sx, sy, k, an, k_offset,timers_enabled), spar::Replicate()]]
		{
			int kk = k_offset + k;
			double t1 = S;
			double t2 = an;
