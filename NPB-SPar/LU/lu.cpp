...
ff::ParallelFor * pf;
int num_workers;
static boolean flag[ISIZ1/2*2+1][1024];

pthread_mutex_t mutex; // = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t cond[1024]; // = PTHREAD_COND_INITIALIZER;

void WAIT_SIGNAL_BUTS(int k, int worker_id) {
    pthread_mutex_lock(&mutex);
	    if (worker_id != num_workers-1) while (flag[k][worker_id+1] == 0) pthread_cond_wait(&cond[worker_id], &mutex);
	pthread_mutex_unlock(&mutex);
}

void GO_SIGNAL_BUTS(int k, int worker_id) {
    pthread_mutex_lock(&mutex);
	    if (worker_id != num_workers-1) flag[k][worker_id+1] = 0;
		if (worker_id != 0) {
			flag[k][worker_id] = 1;  
	    	pthread_cond_signal(&cond[worker_id-1]);
	    }
    pthread_mutex_unlock(&mutex);
}
...
void WAIT_SIGNAL_BLTS(int k, int worker_id) {
    pthread_mutex_lock(&mutex);
	    if (worker_id != 0)  while (flag[k][worker_id-1] == 0) pthread_cond_wait(&cond[worker_id], &mutex);
	pthread_mutex_unlock(&mutex);
}
...
void GO_SIGNAL_BLTS(int k, int worker_id) {
    pthread_mutex_lock(&mutex); 
		...
		if (worker_id != 0) flag[k][worker_id-1] = 0;
    pthread_mutex_unlock(&mutex);
}
...
int main(int argc, char* argv[]){
	...
	if(const char * nw = std::getenv("FF_NUM_THREADS")){
        num_workers = atoi(nw);
    }else{
        num_workers = 1;
    }

    pf = new ff::ParallelFor(num_workers, true);
...
void blts(int nx,
	...
	for(j=jst_worker; j<jend_worker; j++){
		for(i=ist; i<iend; i++){
			for(m=0; m<5; m++){
				v[k][j][i][m]= v[k][j][i][m]
...
void buts(int nx,
	...
	for(j=jend_worker; j>jst_worker; j--){
...
void erhs(){
	 ...
	 * ---------------------------------------------------------------------
	 */
	pf->parallel_for(1, nz-1, 1, [&](int k){
		int i, j, m;
		double q;
		double u21, u31, u41;
		double tmp;
	 ...
	 * ---------------------------------------------------------------------
	 */
	pf->parallel_for(jst, jend, 1, [&](int j){
		int i, k, m;
		double q;
		double u21, u31, u41;
		double tmp;
...
void l2norm(int nx0,
	...
	for(int worker_id=0; worker_id<num_workers; worker_id++){
		for(m=0;m<5;m++){
			sum_worker[worker_id][m]=0.0;
		}
	}
	
	pf->parallel_for_thid(1, nz0-1, 1, 0, [&](int k, int worker_id){
		int j, i, m;
		for(j=jst; j<jend; j++){
			for(i=ist; i<iend; i++){
				for(m=0; m<5; m++){
					sum_worker[worker_id][m] += v[k][j][i][m]*v[k][j][i][m];
				}
			}
		}
	});
...
void read_input(){
	...
	printf("\n\n NAS Parallel Benchmarks 4.1 Parallel C++ version with FastFlow - LU Benchmark\n\n");
...
void ssor(int niter){
	 ...
	 * ---------------------------------------------------------------------
	 */
	pf->parallel_for(0, ISIZ2, 1, [&](int j){
		int i, n, m;
		for(i=0; i<ISIZ1; i++){
			for(n=0; n<5; n++){
				for(m=0; m<5; m++){
	...
	for(istep=1; istep<=niter; istep++){
		...
		if(timeron){timer_stop(T_RHS);}
		pf->parallel_for(0, num_workers, 1, 1, [&](int worker_id){
			int k;
			for(k=1; k<nz-1; k++){
				/*
				 * ---------------------------------------------------------------------
